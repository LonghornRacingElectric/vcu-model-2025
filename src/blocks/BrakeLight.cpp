...
void BreakLight::evaluate(VcuParameters *params, BreakLightInput *input,
    ...
    bool hardBraking = accumulator > params->brakesHardThreshold ||
                       accumulator < -params->brakesHardThreshold;
    bool braking = accumulator > params->brakesSoftThreshold ||
                   accumulator < -params->brakesSoftThreshold;

    if (hardBraking) {
        bool pattern = (brakeTimer > 0.5f) ||
                       (static_cast<uint32_t>(brakeTimer / 0.05f) % 2 == 0);
