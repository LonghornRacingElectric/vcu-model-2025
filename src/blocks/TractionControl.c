 ...
 * This prevents the car from spinning out, giving the driver more confidence in
 * acceleration.
 */

void TractionControl_setParameters(TractionControl *tractionControl,
                                   VcuParameters *params) {
    LowPassFilter_init(&tractionControl->lowPassFeedback,
                       params->tcsFeedbackLowPassFilterTimeConstant);
    // tractionControl->lowPassFeedback =
    // LowPassFilter(params->tcsFeedbackLowPassFilterTimeConstant);
}

void TractionControl_evaluate(TractionControl *tractionControl,
                              VcuParameters *params,
                              TractionControlInput *input,
                              TractionControlOutput *output, float deltaTime) {
    if (!params->tcsEnabled || !input->wheelSpeedsOk) {
        output->regulatedTorqueRequest = input->unregulatedTorqueRequest;
        return;
    }

    float averageFrontVelocity =
        (input->wheelSpeedFl + input->wheelSpeedFr) / 2.0f;
    ...
    float excessVelocity =
        (excessVelocityL > excessVelocityR) ? excessVelocityL : excessVelocityR;
    ...
    LowPassFilter_add(&tractionControl->lowPassFeedback, excessVelocity,
                      deltaTime);
